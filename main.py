...
import time as tm
from telebot import types
from utils2.stocks import *
from utils2.crypto_coins import *
import requests
catalog_dict = {}
product_dict = {}
download_dict = {}
...
def start_bot():
    ...
    def handler_start(message):
        ...
        bot.send_photo(chat_id, 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWy-8CQ6QqDBqsZ749xiyVBySa0X8f8Tc7IA&usqp=CAU')
        bot.send_message(chat_id,
                         text='❇️  Добро пожаловать {}! \nУнас самок лучьшее качество  🧊  \nи по доступным ценам !!'.format(message.from_user.first_name),
    ...
    def handler_call(call):
        ...
        if call.data == 'Bitcoin':
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text=(f"💰Bitcoin:   {btc_price_Bitcoin}"),
                reply_markup=menu.main_menu
            )
        if call.data == 'Litecoin':
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text=(f"💰Litecoin:   {btc_price_Litecoin}"),
                reply_markup=menu.main_menu
            )            
        if call.data == 'Ethereum':
            bot.edit_message_text(
                chat_id=chat_id,
                message_id=message_id,
                text=(f"💰Ethereum:   {btc_price_Ethereum}"),
                reply_markup=menu.main_menu
            )            
